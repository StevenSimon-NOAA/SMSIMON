...
mkdir -p ${OUTPUT_PATH}
mkdir -p ${DATA_PATH}/untar_analyses/untar_gfs_f000
...
mkdir -p ${DATA_PATH}/untar_analyses/untar_gfs_f000/gfs.${VYYYYMMDD}/${VHH}
if [ ! -d ${DATA_PATH}/gfs.${VYYYYMMDD}/${VHH}/atmos ]; then
...
if ((${VALID} >= ${GFS_CHANGE_DATE6})) ; then
	...
	GFS_FILENAME=./gfs.${VYYYYMMDD}/${VHH}/atmos/gfs.t${VHH}z.pgrb2.1p00.f000
	...
	GFS_ARCHIVE=/NCEPPROD/hpssprod/runhistory/rh${VYYYY}/${VYYYYMM}/${VYYYYMMDD}/gpfs_hps_nco_ops_com_gfs_prod_gfs.${VALID}.pgrb2_1p00.tar
        GFS_FILENAME=./gfs.t${VHH}z.pgrb2.1p00.f000
...
cat > ${DATA_PATH}/untar_analyses/untar_gfs_f000/gfs.${VYYYYMMDD}/${VHH}/htar_gfs_f000_${VALID}.sh <<EOF
#!/bin/bash
#PBS -N gfs_htar
#PBS -o ${OUTPUT_PATH}/out_htar_gfs_f000_${VALID}.out
#PBS -e ${OUTPUT_PATH}/out_htar_gfs_f000_${VALID}.err
#PBS -l select=1:ncpus=1:mem=4GB
#PBS -q dev_transfer
#PBS -l walltime=03:00:00
#PBS -A VERF-DEV

cd ${DATA_PATH}/untar_analyses/untar_gfs_f000/gfs.${VYYYYMMDD}/${VHH}

if [[ -s ${DATA_PATH}/gfs.${VYYYYMMDD}/${VHH}/atmos/gfs.t${VHH}z.pgrb2.1p00.f000 ]] ; then
	echo ${VALID}" GFS F000 already exists"
else
	echo "Extracting "${VALID}" GFS F000"
	htar -xvf $GFS_ARCHIVE $GFS_FILENAME
	sleep 3
	if test ${GFS_FILENAME} = "./gfs.t${VHH}z.pgrb2.0p25.f000" ; then
		echo ${VALID}" GFS F000 file is only avaiable in 0p25"
		mv ${GFS_FILENAME} ${DATA_PATH}/gfs.${VYYYYMMDD}/${VHH}/atmos/gfs.t${VHH}z.pgrb2.0p25.f000
	else
                echo ${VALID}" GFS F000 file is avaiable in 1p00"
		mv ${GFS_FILENAME} ${DATA_PATH}/gfs.${VYYYYMMDD}/${VHH}/atmos/gfs.t${VHH}z.pgrb2.1p00.f000
	fi
fi
...
qsub ${DATA_PATH}/untar_analyses/untar_gfs_f000/gfs.${VYYYYMMDD}/${VHH}/htar_gfs_f000_${VALID}.sh
sleep 3
